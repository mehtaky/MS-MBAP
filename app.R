...
list.of.packages <- c('shiny', 'shinydashboard', 'foreach', 'dplyr', 'reshape2', 'ggplot2', 'equatiomatic', 'zeallot', 'plotly', 'scales', 'xlsx2dfs', 'shinybusy', 'tibble', 'readr', 'hydroGOF', 'stringr', 'BiocManager', 'UpSetR', 'JADE', 'corpcor')
...
new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
if(length(new.packages)) devtools::install_github("dengkuistat/WaveICA_2.0")
...
server <- function(input, output, session) {
  ...
  observeEvent(input$CorrectBatchEffect, {
    ...
    if (!is.null(workingdatasets$imputed_dataset)) {
      ...
      dataset <- t(alignedFeatures$interested_features_dataframe[,-1:-ncol(all_aligned_features)])
      
      colnames(dataset) <- paste(round(alignedFeatures$interested_features_dataframe[,2],2), round(alignedFeatures$interested_features_dataframe[,1],4) ,sep = "_")
    }
    
    
    dataset <- dataset[order(match(sapply(strsplit(rownames(dataset), "[.]"), "[", 2), additional_info()[,1])),]
    
    dataset_WaveICA_corrected <- WaveICA2.0::WaveICA_2.0(dataset, Injection_Order = additional_info()[,4], alpha = 0, Cutoff = 0.1, K=10)
